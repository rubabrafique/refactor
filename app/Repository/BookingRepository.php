...
use DATApi\Helper\CommonHelper;
use DATApi\Helper\NotificationHelper;
...
class BookingRepository extends BaseRepository
{
    ...
    function __construct(Job $model, MailerInterface $mailer)
    {
        ...
        $this->translatorRepo = new TranslatorRepository(new Translator());
    ...
    {
        $cuser = $this->userRepo->getUserById($user_id);
        $usertype = '';
        $emergencyJobs = array();
        $noramlJobs = array();
    ...
    private function changePendingStatus($job, $data, $changedTranslator)
    {
        ...
        if ($data['status'] == 'assigned' && $changedTranslator) {
            ...
            $this->notificationHelper->sendSessionStartRemindNotification($user, $job, $language, $job->due, $job->duration);
            $this->notificationHelper->sendSessionStartRemindNotification($translator, $job, $language, $job->due, $job->duration);
            return true;
        } else {
    ...
    private function changeTranslator($current_translator, $data, $job)
    {
        ...
        if (!is_null($current_translator) || (isset($data['translator']) && $data['translator'] != 0) || $data['translator_email'] != '') {
            ...
            if (!is_null($current_translator) && ((isset($data['translator']) && $current_translator->user_id != $data['translator']) || $data['translator_email'] != '') && (isset($data['translator']) && $data['translator'] != 0)) {
                ...
                $new_translator = $this->translatorRepo->createTranslator($new_translator);
                ...
                if ($data['translator_email'] != '') $data['translator'] = User::where('email', $data['translator_email'])->first()->id;
                $new_translator =  $this->translatorRepo->createTranslator(['user_id' => $data['translator'], 'job_id' => $job->id]);
                $log_data[] = [
                    'old_translator' => null,
                    'new_translator' => $new_translator->user->email
                ];
    ...
    public function sendExpiredNotification($job, $user)
    {
        ...
        if ($this->notificationHelper->isNeedToSendPush($user->id)) {
            $users_array = array($user);
            $this->notificationHelper->sendPushNotificationToSpecificUsers($users_array, $job->id, $data, $msg_text, $this->notificationHelper->isNeedToDelayPush($user->id));
        }
    }
    ...
    public function acceptJobWithId($job_id, $cuser)
    {
        ...
        if (!Job::isTranslatorAlreadyBooked($job_id, $cuser->id, $job->due)) {
            if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $job_id)) {
                ...
                if ($this->notificationHelper->isNeedToSendPush($user->id)) {
                    $users_array = array($user);
                    $this->notificationHelper->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->notificationHelper->isNeedToDelayPush($user->id));
                }
        ...
        */
        ...
        if ($cuser->is('customer')) {
            ...
            if ($translator) {
                ...
                if ($this->notificationHelper->isNeedToSendPush($translator->id)) {
                    $users_array = array($translator);
                    $this->notificationHelper->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->notificationHelper->isNeedToDelayPush($translator->id));// send Session Cancel Push to Translaotor
                }
            }
        } else {
            if ($job->due->diffInHours(Carbon::now()) > 24) {
                ...
                if ($customer) {
                    ...
                    if ($this->notificationHelper->isNeedToSendPush($customer->id)) {
                        $users_array = array($customer);
                        $this->notificationHelper->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->notificationHelper->isNeedToDelayPush($customer->id));     // send Session Cancel Push to customer
                    }
                }
                $job->status = 'pending';
                $job->created_at = date('Y-m-d H:i:s');
...
//        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
        ...
        $this->translatorRepo->where('job_id', $jobid)->where('cancel_at', NULL)->update(['cancel_at' => $data['cancel_at']]);
        $Translator =  $this->translatorRepo->createTranslator($data);
    ...
    public function updateDistance($jobId, $data) {
       return Distance::where('job_id', '=', $jobId)->update(array('distance' => $data["distance"], 'time' => $data["time"]));
    }

    public function updateJobInfo($jobId, $data) {
       return Job::where('id', '=', $jobId)->update(array_except($data, ['distance', 'time', 'jobid']));
    }
}